...
import './InstrumentCategoryFormComponent.scss';
import ViewFormComponent from "./ViewFormComponent";

export default function InstrumentCategoryFormComponent() {
  const mode = decryptData(useParams().mode);
  const { t } = useTranslation(["Common", "Messages", "Form"]);
  const breadcrumbItems = [
   
    {
      label: t("Common:App_lms_Common_00272"),
      href: "/" + encrypt("InstrumentTypeList"),
