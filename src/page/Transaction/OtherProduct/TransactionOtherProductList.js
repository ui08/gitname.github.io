...
export default function TransactionOtherProductList({ UploadModalOpen }) {
      ...
      try {
        const response = await axiosInstance.get(
          Apiurl.globalSearchTransaction +
            ...
            "/" +
            filterData.label +
            `?insType=OTHER_PRODUCTS&page=${currentPage}&size=${perPagesize}`
        );
  ...
  const fetchFilter = async () => {
    try {
      const response = await axiosInstance.get(
        Apiurl.globalSearchFilter + "/" + "TRANSACTION_OTHER_PRODUCT"
      );
      let result = response.data;
      setAllFilter([]);
