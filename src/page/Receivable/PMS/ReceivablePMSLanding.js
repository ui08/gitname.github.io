...
import ReceivablePMSCommissionReceiveables from "./ReceivablePMSCommissionReceiveables";
import ReceivablePMSList from "./ReceivablePMSList";
import { ReceivablePMSPageurl } from "./ReceivablePMSPageurl";

export default function ReceivablePMSLanding() {
  ...
  const handleAddNew = () => {
    navigate(
      "/" +
        encrypt("ReceivablePMSFormComponent") +
        `/${encryptData("add")}` +
        `/${encryptData("ss")}`
    );
  };
        ...
        <>
          <Pagehader
            ...
            breadcrumbItems={
              pageName === "CommissionMaster"
                ? ReceivablePMSPageurl.OtherProductsList
                : ReceivablePMSPageurl.OtherProductsHistory
            }
          ></Pagehader>
          <div className="pagebody">
            ...
            <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xl-12">
              {pageName === "CommissionMaster" && <ReceivablePMSList />}
              {pageName === "CommissionReceiveables" && (
                <ReceivablePMSCommissionReceiveables />
              )}
            </div>
          </div>
        </>
