...
import ReceivableMutualFundList from "./ReceivableMutualFundList";
import { ReceivableMutualFundPageurl } from "./ReceivableMutualFundPageurl";
import TransactionOtherProductCommissionReceiveables from './TransactionOtherProductCommissionReceiveables';

export default function ReceivableMutualFundLanding() {
  ...
  const handleCommission = () => {
    navigate(
      "/" +
        encrypt("ReceivableMutualFundLanding") +
  ...
  const handleAddNew = () => {
    navigate(
      "/" +
        encrypt("ReceivableMutualFundFormComponent") +
        `/${encryptData("add")}` +
        `/${encryptData("ss")}`
    );
  };
        ...
        <>
          <Pagehader
            ...
            breadcrumbItems={
              pageName === "CommissionMaster"
                ? ReceivableMutualFundPageurl.OtherProductsList
                : ReceivableMutualFundPageurl.OtherProductsHistory
            }
          ></Pagehader>
          <div className="pagebody">
            ...
            <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xl-12">
              {pageName === "CommissionMaster" && <ReceivableMutualFundList />}
              {pageName === "CommissionReceiveables" && (
                <TransactionOtherProductCommissionReceiveables />
              )}
            </div>
          </div>
        </>
