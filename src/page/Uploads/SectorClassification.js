...
export default function SectorClassification() {
  ...
  useEffect(() => {
    ...
    // getUserDetails();
    handleRefresh(uploadEnum.SECTOR_CLASSIFICATION);
  }, []);
  ...
  const handledownloadTemplate = (data) => {
    DownloadTemplateFunction("SECTOR_CLASSIFICATION");
  };
  const handleFileUpload = (file) => {
    ...
    let fileUploadDTO = JSON.stringify({
      filename: "SECTOR_CLASSIFICATION",
      userId: getUserId(),
      fileUploadType: uploadEnum.SECTOR_CLASSIFICATION,
    });
  ...
  const handleRefresh = async () => {
    try {
      const response = await axiosInstance.get(
        `${Apiurl.uploadHistory}${uploadEnum.SECTOR_CLASSIFICATION}`
      );
      if (response) {
  ...
  return (
    <>
      ...
      <div className="row">
        {/* <Historycard data={tenantMasterData} type = {uploadEnum.SECTOR_CLASSIFICATION} name = {userName}/> */}
        <AgTable
