...
export default function HoldingUnderlying() {
  ...
  useEffect(() => {
    ...
    // getUserDetails();
    handleRefresh(uploadEnum.HOLDING_UNDERLYING);
  }, []);
  ...
  const handledownloadTemplate = (data) => {
    DownloadTemplateFunction("HOLDING_UNDERLYING");
  };
  const handleFileUpload = (file) => {
    ...
    let fileUploadDTO = JSON.stringify({
      filename: "HOLDING_UNDERLYING",
      userId: getUserId(),
      fileUploadType: uploadEnum.HOLDING_UNDERLYING,
    });
  ...
  const handleRefresh = async () => {
    try {
      const response = await axiosInstance.get(
        `${Apiurl.uploadHistory}${uploadEnum.HOLDING_UNDERLYING}`
      );
      if (response) {
    ...
    <>
      ...
      <div className="row">
        {/* <Historycard
          ...
          type={uploadEnum.HOLDING_UNDERLYING}
          name = {userName}
        /> */}
        <AgTable
