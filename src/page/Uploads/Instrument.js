...
export default function Instrument() {
  ...
  useEffect(() => {
    ...
    // getUserDetails();
    handleRefresh(uploadEnum.CREATE_INSTRUMENT);
  }, []);
  ...
  const handledownloadTemplate = (data) => {
    DownloadTemplateFunction("CREATE_INSTRUMENT");
  };
  ...
  const handleFileUpload = (file) => {
    ...
    let fileUploadDTO = JSON.stringify({
      filename: "INSTRUMENT",
      userId: getUserId(),
      fileUploadType: uploadEnum.CREATE_INSTRUMENT,
    });
  ...
  const handleRefresh = async () => {
    try {
      const response = await axiosInstance.get(
        `${Apiurl.uploadHistory}${uploadEnum.CREATE_INSTRUMENT}`
      );
      if (response) {
    ...
    <>
      ...
      <div className="row">
        {/* <Historycard
          ...
          type={uploadEnum.CREATE_INSTRUMENT}
          name = {userName}
        /> */}
        <AgTable
