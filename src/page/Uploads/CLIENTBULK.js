...
import { UploadTemplateFunction } from "./UploasAction";
import { uploadEnum } from "./data";

export default function CLIENTBULK() {
  ...
  useEffect(() => {
    ...
    getUserDetails();
    handleRefresh(uploadEnum.CLIENT_MASTER_BULK_UPLOAD);
  }, []);
  ...
  const handledownloadTemplate = (data) => {
    CLIENTDownloadTemplateFunction();
  };
  const CLIENTDownloadTemplateFunction = async () => {
    try {
      const response = await axiosInstance.get(
        `${Apiurl.clientMstOnboardingExcelTemplate}`,
        {
          responseType: "blob",
        }
      );
  ...
  const handleFileUpload = (file) => {
    ...
    let fileUploadDTO = JSON.stringify({
      filename: "CLIENT_MASTER_BULK_UPLOAD",
      userId: userId,
    ...
    <>
      ...
      <div className="row">
        {/* <Historycard
          ...
          type={uploadEnum.CLIENT_MASTER_BULK_UPLOAD}
          name = {userName}
        /> */}
        <AgTable
