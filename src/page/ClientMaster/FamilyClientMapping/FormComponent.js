...
import "./FamilyClientMappingFormComponent.scss";
...
const FormComponent = ({
  ...
  const fetchAllFamily = async (value) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get(Apiurl.getAllFamiliesDropdown);
      setLoading(false);
      setAllFamily([]);
      Object.values(response.data).map((item) => {
        if (value == item.id) {
          setValue("FamilyName", { label: item.name, value: item.familyCode });
        }
        let SingleData = {
          label:  item.name,
          value: item.id,
          familyHeadId : item.mappedClientsId
        };
