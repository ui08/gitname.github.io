...
const fetchData = async () => {
  try {
    const response = await axiosInstance.get(`${Apiurl.familyClientMappingSingleView}/${id}`);
    ...
    console.log("kk",data);
    let clientId = data.ClientName.map((item)=> item.value)
    let PayloadData = {
     familyHeadClientId: data.FamilyName.familyHeadId,
    familyMasterId: data.FamilyName.value,
    memberClientIds: clientId,
    relations: [data.Relation.label]
    };
