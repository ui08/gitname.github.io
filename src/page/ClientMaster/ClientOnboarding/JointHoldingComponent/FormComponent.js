...
const FormComponent = ({
  ...
  useEffect(() => {
    if (initialData) {
      GetallGenderList(initialData?.clientJointHolderDetailsEntityDto[0].gender);
      GetallHoldingTypeList(initialData?.clientJointHolderDetailsEntityDto[0].holding);
      GetallCountry(initialData?.clientJointHolderDetailsEntityDto[0].countryOfBirth, "country");
      GetallCountry(initialData?.clientJointHolderDetailsEntityDto[0].taxResidentCountry, "tax");
      GetOccupationList(initialData?.clientJointHolderDetailsEntityDto[0].occupation);
      GetWealthSourceList(initialData?.clientJointHolderDetailsEntityDto[0].wealthSource);
      GetIncomeSlabList(initialData?.clientJointHolderDetailsEntityDto[0].incomeSlab);
      GetIdentificationTypeList(initialData?.clientJointHolderDetailsEntityDto[0].identificationType)
      ...
      setValue("PoliticalExposureType1", {label : initialData?.clientJointHolderDetailsEntityDto[0].politicalExposureType, value : initialData?.clientJointHolderDetailsEntityDto[0].politicalExposureType == "Yes" ? 1 : 2 })
      reset(initialData);
    } else {
      GetallGenderList();
      ...
      <div className="row formMainDiv">
        ...
        >
          <InputText
            ...
            name={"OtherGender1"}
            mandatory={showOtherGender1 ? true : false}
            onPaste={false}
            onCopy={false}
            minLength={1}
            maxLength={30}
            ...
            onChange={() => {}}
            divClassName={
              showOtherGender1 ? "divClassName" : "divClassName  d-none"
            }
          />
        </div>
