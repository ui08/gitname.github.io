...
export default function TransactionMutualFundsList({ UploadModalOpen }) {
 ...
 const [filterData, setFilterData] = useState({label : "", value : ""});
  ...
  const createRowData = (value) => {
    ...
    Object.values(value?.content).map((item, index) => {
      let SingleData = {
        ...
        trxnDisc: item.trxnDisc === null ? `-` : item.trxnDisc,
        transactionDate:
        ...
        folioNumber: item.folioNumber === null ? `-` : item.folioNumber,
        serviceProviderName:
        ...
        stampDuty: item.stampDuty === null ? `-` : item.stampDuty,
        brokerageAmount:
      ...
      const handleSearchChange = async (value) => {
        ...
        try {
          const response = await axiosInstance.get(
            Apiurl.globalSearchTransaction  + value + "/" + filterData.label + `?insType=MUTUAL_FUND&page=${currentPage}&size=${perPagesize}`
          );
  ...
  const useFromProps = {
   ...
   const fetchFilter = async () => {
        try {
          const response = await axiosInstance.get(
            Apiurl.globalSearchFilter + "/" + "TRANSACTION_MUTUAL_FUND"
          );
          let result = response.data;
          setAllFilter([])
